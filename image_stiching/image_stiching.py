...
import glob
path = sorted(glob.glob("obrazky/1/*.jpg"))
img_list = []
for img in path:
    n = cv2.imread(img)
    img_list.append(n)
...
# Set minimum match condition
  ...
  if len(good) > MIN_MATCH_COUNT:
    ...
    #result_with_keypoints = cv2.drawKeypoints(result, keypoints2, None, color=(0, 255, 0), flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
...
cv2.imwrite("image_stiching_1.png", result)

#plt.ylim(800,0)
plt.imshow(result)
plt.show()

"""na odstranenie čierneho fillu
...
# Apply mask to image
result_mask = cv2.bitwise_and(result, result, mask=mask)
